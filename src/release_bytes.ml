...
module type ByteOps = sig
  type t
  val read_at : int -> B.t -> t
  val write_byte : t -> B.t -> unit
  val write : t -> B.t -> unit
end
...
module type IntegerOps = sig
  ...
  val read_int16_at : int -> B.t -> int
  val read_int16 : B.t -> int
  val write_int16_byte : int -> B.t -> unit
  val write_int16 : int -> B.t -> unit

  val read_int_at : int -> B.t -> int
  val read_int : B.t -> int
  val write_int_byte : int -> B.t -> unit
  val write_int : int -> B.t -> unit

  val read_int32_at : int -> B.t -> int32
  val read_int32 : B.t -> int32
  val write_int32_byte : int32 -> B.t -> unit
  val write_int32 : int32 -> B.t -> unit

  val read_uint32_at : int -> B.t -> Uint32.t
  val read_uint32 : B.t -> Uint32.t
  val write_uint32_byte : Uint32.t -> B.t -> unit
  val write_uint32 : Uint32.t -> B.t -> unit

  val read_int64_at : int -> B.t -> int64
  val read_int64 : B.t -> int64
  val write_int64_byte : int64 -> B.t -> unit
  val write_int64 : int64 -> B.t -> unit

  val read_uint64_at : int -> B.t -> Uint64.t
  val read_uint64 : B.t -> Uint64.t
  val write_uint64_byte : Uint64.t -> B.t -> unit
  val write_uint64 : Uint64.t -> B.t -> unit

  val read_uint128_at : int -> B.t -> Uint128.t
  val read_uint128 : B.t -> Uint128.t
  val write_uint128_byte : Uint128.t -> B.t -> unit
  val write_uint128 : Uint128.t -> B.t -> unit
