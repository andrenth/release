(** This module defines two implementations of a buffer type and a set of
    operations on buffers. [Release_buffer.String] implements buffers using
    OCaml's [string] type, while [Release_buffer.Byte] is implemented based on
    the [Lwt_buffer.t] type, which on its turn is implemented using OCaml's
    [Bigarray] module.
*)

module type S = sig
  (** The signature of [Release_buffer] buffer modules. *)
...
module String : S
  (** Buffer module implemented on top of OCaml's [string] type. *)

module Bytes : S
  (** Buffer module implemented on top of Lwt's [Lwt_bytes.t] type. *)
