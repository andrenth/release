...
module String = struct
  ...
  let create n =
    { bytes = String.create n; len = 0 }

  let make n c =
    { bytes = String.make n c; len = n }
  ...
  let add_string buf s =
    let len = String.length s in
    String.blit s 0 buf.bytes buf.len len;
    buf.len <- buf.len + len

  let sub buf off len =
    let b = String.sub buf.bytes off len in
    { bytes = b; len = len }

  let blit buf1 off1 buf2 off2 len =
    String.blit buf1.bytes off1 buf2.bytes off2 len;
    buf2.len <- max buf2.len (off2 + len)
  ...
  let read fd buf off len =
    lwt k = Lwt_unix.read fd buf.bytes off len in
    buf.len <- max buf.len (off + k);
    return k

  let write fd buf off len =
    Lwt_unix.write fd buf.bytes off len
end
